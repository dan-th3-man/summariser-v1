...
class SupabaseService {
    ...
    async getMessagesByDateRange(startDate, endDate, discordServerId, channelId // Optional Discord channel ID
    ) {
    ...
    async debugReactions(serverId, channelId) {
        ...
        const emojiReactions = possibleReactions?.filter(r => r.content?.text?.match(/^\*<[^>]+>: ".*"\*$/) &&
            r.content?.inReplyTo) || [];
